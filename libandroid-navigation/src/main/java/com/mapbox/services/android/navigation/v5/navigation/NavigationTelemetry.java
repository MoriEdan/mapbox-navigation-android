...
class NavigationTelemetry implements NavigationMetricListener {
  ...
  private void fireOffBatteryScheduler() {
    ...
    TimerTask batteryTask = new TimerTask() {
      @Override
      public void run() {
        BatteryChargeFinder batteryChargeFinder = new BatteryChargeFinder();
        float batteryPercentage = batteryChargeFinder.obtainPercentage(context);
        BatteryEvent batteryEvent = new BatteryEvent(navigationSessionState.sessionIdentifier(), batteryPercentage);
